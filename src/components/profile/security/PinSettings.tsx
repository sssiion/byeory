    ...
    const handlePinSubmit = async (pin: string): Promise<string | null> => {
        ...
        try {
            if (pinModalConfig.mode === 'REGISTER') {
                const response = await fetch(URL+'/api/pin/register', {
                    method: 'POST',
                    headers: {
                ...
                const response = await fetch(URL+'/api/pin/verify', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
                ...
                if (isCorrect === true) {
                    ...
                    const statusRes = await fetch(URL+'/api/pin/status', {
                ...
                const verifyResponse = await fetch(URL+'/api/pin/verify', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
                ...
                if (isCorrect === true) {
                    const deleteResponse = await fetch(URL+'/api/pin', {
                        method: 'DELETE',
                        headers: { 'Authorization': `Bearer ${token}` }
                    });

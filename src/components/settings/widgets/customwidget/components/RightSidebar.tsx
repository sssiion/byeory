...
const RightSidebar: React.FC<Props> = ({ selectedBlock, onUpdateBlock }) => {
    ...
    // 🌟 [NEW] KOFIC API Key (로컬 스토리지 사용 권장)
    const [koficKey, setKoficKey] = React.useState(localStorage.getItem('kofic_api_key') || '');

    const saveKoficKey = (key: string) => {
        setKoficKey(key);
        localStorage.setItem('kofic_api_key', key);
    };

    // 영화 검색 함수 (KOFIC Open API 사용)
    const searchMovies = async () => {
        if (!movieQuery.trim()) return;
        if (!koficKey) {
            alert('KOFIC API 키가 필요합니다. 설정에서 키를 입력해주세요.');
            return;
        }
        ...
        try {
            ...
            const url = `http://www.kobis.or.kr/kobisopenapi/webservice/rest/movie/searchMovieList.json?key=${koficKey}&movieNm=${encodeURIComponent(movieQuery)}&itemPerPage=10`;

            const res = await fetch(url);
            const data = await res.json();

            const list = data.movieListResult?.movieList || [];
            console.log("KOFIC Movie Results:", list);
            setMovieResults(list);
            ...
            alert('영화 검색에 실패했습니다. (API 키를 확인하거나 네트워크 상태를 점검하세요)');
        } finally {
    ...
    return (
        <aside className="w-80 bg-[var(--bg-card)] border-l border-[var(--border-color)] flex flex-col text-sm h-full">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin scrollbar-thumb-[var(--border-color)]">
                ...
                {/* --- 🌟 [Phase 2] Action Settings --- */}
                {(type === 'button') && (
                    <div className="space-y-4 pb-4 border-b border-[var(--border-color)]">
                        <Label>ACTION</Label>
                        <div className="space-y-1">
                            <span className="text-xs text-[var(--text-secondary)]">링크 URL</span>
                            <Input
                                value={selectedBlock.action || ''}
                                onChange={(val: string) => onUpdateBlock(selectedBlock.id, { action: val })}
                                placeholder="https://example.com"
                            />
                            <p className="text-[10px] text-[var(--text-secondary)]">클릭 시 해당 링크로 이동합니다.</p>
                        </div>
                    </div>
                )}

                {/* --- 🌟 [Phase 3] Chart Data Settings --- */}
                {(type === 'chart-pie' || type === 'chart-bar') && (
                    <div className="space-y-4 pb-4 border-b border-[var(--border-color)]">
                        ...
                        {(content.data || []).map((item: any, idx: number) => (
                            <div key={idx} className="flex gap-2 items-center">
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        const newData = [...(content.data || [])];
                                        newData[idx] = { ...item, value: Number(e.target.value) };
                                        updateContent('data', newData);
                                    }}
                                    className="w-16 bg-[var(--bg-card-secondary)] text-[var(--text-primary)] p-2 rounded border border-[var(--border-color)] outline-none text-xs"
                                />
                                <button
                                    onClick={() => {
                                        const newData = [...(content.data || [])];
                        ...
                        <button
                            onClick={() => {
                                const newData = [...(content.data || []), { label: 'New', value: 10 }];
                                updateContent('data', newData);
                            }}
                            className="w-full py-2 bg-[var(--bg-card-secondary)] hover:bg-[var(--bg-card-hover)] text-[var(--text-primary)] rounded text-xs border border-[var(--border-color)]"
                        >
                            + 데이터 추가
                        </button>
                    </div>
                ...
                {/* 🌟 [Phase 3] Progress Bar Settings */}
                {type === 'progress-bar' && (
                    <div className="space-y-4 pb-4 border-b border-[var(--border-color)]">
                        <Label>PROGRESS SETTINGS</Label>
                        <div className="space-y-1">
                            <span className="text-xs text-[var(--text-secondary)]">라벨</span>
                            <Input
                                value={content.label || ''}
                ...
                {/* --- 1. 콘텐츠 설정 (타입별 분기) --- */}
                {/* 🌟 [Refinement] Hide duplicate data section for charts */}
                {!['chart-pie', 'chart-bar', 'progress-bar'].includes(type) && (
                    <div className="space-y-4">
                        <Label>DATA & CONTENT</Label>

                        {/* A. 기본 텍스트류 */}
                        {['heading1', 'heading2', 'heading3', 'text', 'typing-text', 'quote', 'callout', 'spoiler', 'highlight', 'vertical-text', 'scroll-text'].includes(type) && (
                            <TextArea
                                value={content.text}
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="pt-2 border-t border-[var(--border-color)]/30 mt-2">
                                            <div className="flex justify-between items-center mb-1">
                                                <span className="text-[10px] text-[var(--text-secondary)]">KOFIC API Key</span>
                                                <a href="https://www.kobis.or.kr/kobisopenapi/homepg/apiservice/searchServiceInfo.do" target="_blank" rel="noreferrer" className="text-[10px] text-indigo-400 hover:underline">
                                                    키 발급받기
                                                </a>
                                            </div>
                                            <input
                                                type="password"
                                                value={koficKey}
                                                onChange={(e) => saveKoficKey(e.target.value)}
                                                placeholder="API 키 입력"
                                                className="w-full bg-[var(--bg-card)] text-[var(--text-primary)] p-1.5 rounded border border-[var(--border-color)] outline-none text-[10px]"
                                            />
                                        </div>
                                        ...
                                        <div className="text-center pt-2">
                                            <button
                                                onClick={() => {
                                                    // 빈 데이터로 영화 정보 생성 -> 편집 모드로 전환
                                                    onUpdateBlock(selectedBlock.id, {
                                                        content: {
                                                            ...content,
                                                            movieData: {
                                                                title: movieQuery || '제목 없음',
                                        ...
                                        {movieResults.length > 0 && (
                                            <div className="space-y-2 max-h-60 overflow-y-auto pr-1 scrollbar-thin border-t border-[var(--border-color)] pt-2">
                                                {movieResults.map((m: any) => {
                                                    // KOFIC 데이터 매핑
                                                    // KOFIC은 포스터 이미지를 제공하지 않음
                                                    const title = m.movieNm;
                                                    const titleEn = m.movieNmEn;
                                                    const year = m.prdtYear || m.openDt?.substring(0, 4) || '';
                                                    const directors = (m.directors || []).map((d: any) => d.peopleNm).join(', ');
                                                    const genre = m.genreAlt;
                                                    ...
                                                    return (
                                                            ...
                                                            onClick={() => {
                                                                onUpdateBlock(selectedBlock.id, {
                                                                    content: {
                                                                        ...
                                                                        movieData: {
                                                                            title: title,
                                                                            poster: 'https://via.placeholder.com/300x450?text=No+Poster', // 포스터 없음
                                                                            year: year,
                                                                            director: directors,
                                                                            plot: `${genre} / ${m.nationAlt}` // 줄거리 대신 장르/국가 정보 표시
                                                                        }
                                                                    }
                                                                });
                                                                setMovieResults([]);
                                                        ...
                                                        >
                                                            {/* 포스터 플레이스홀더 */}
                                                            <div className="w-8 h-12 bg-gray-800 rounded flex items-center justify-center text-[8px] text-gray-500 text-center leading-tight p-0.5">
                                                                No<br />Img
                                                            </div>
                                                            <div className="flex-1 min-w-0 flex flex-col justify-center">
                                                                <div className="text-xs font-bold text-gray-200 truncate">{title}</div>
                                                                <div className="text-[10px] text-[var(--text-secondary)] truncate">
                                                                    {year} · {directors || '감독 미상'}
                                                                </div>
                                                            </div>
                                                        </div>
                                                    );
                ...
                {/* --- 3. 스타일 디테일 (Border, Radius, Padding) --- */}
                <div className="space-y-4 pt-4 border-t border-[var(--border-color)]">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <span className="text-xs text-[var(--text-secondary)]">모서리 둥글게 (Radius)</span>
                            <div className="flex items-center gap-2">
                                <input
                                    type="range" min="0" max="50"
                                    value={styles.borderRadius || 0}
                                    onChange={(e) => onUpdateBlock(selectedBlock.id, { styles: { ...styles, borderRadius: Number(e.target.value) } })}
                                    className="flex-1 h-1.5 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                                />
                                <span className="text-[10px] w-6 text-right">{styles.borderRadius || 0}</span>
                            </div>
                        </div>
                        <div className="space-y-1">
                            <span className="text-xs text-[var(--text-secondary)]">여백 (Padding)</span>
                            <div className="flex items-center gap-2">
                                <input
                                    type="range" min="0" max="40"
                                    value={styles.padding || 0}
                                    onChange={(e) => onUpdateBlock(selectedBlock.id, { styles: { ...styles, padding: Number(e.target.value) } })}
                                    className="flex-1 h-1.5 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                                />
                                <span className="text-[10px] w-6 text-right">{styles.padding || 0}</span>
                            </div>
                        </div>
                    </div>

                    {/* 3. 테두리 (Border Detail) */}
                    <div className="space-y-2 bg-[var(--bg-card-secondary)] p-3 rounded-lg border border-[var(--border-color)]">
                        <div className="flex justify-between items-center">
                            <span className="text-xs font-bold text-[var(--text-secondary)]">테두리 설정</span>
                            <select
                                value={styles.borderStyle || 'none'}
                                onChange={(e) => onUpdateBlock(selectedBlock.id, { styles: { ...styles, borderStyle: e.target.value as any } })}
                        ...
                        {styles.borderStyle && styles.borderStyle !== 'none' && (
                            <>
                                <div className="space-y-1">
                                    <span className="text-[10px] text-[var(--text-secondary)]">두께 (Width)</span>
                                    <div className="flex items-center gap-2">
                                        <input
                                            type="range" min="1" max="10"
                                            value={styles.borderWidth || 1}
                                            onChange={(e) => onUpdateBlock(selectedBlock.id, { styles: { ...styles, borderWidth: Number(e.target.value) } })}
                                            className="flex-1 h-1.5 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                                        />
                                        <span className="text-[10px] w-4">{styles.borderWidth || 1}</span>
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    <span className="text-xs text-[var(--text-secondary)]">색상 (Color)</span>
                                    <div className="flex gap-2 flex-wrap">
                                        {['#000', '#666', '#e5e7eb', '#ef4444', '#f59e0b', '#10b981', '#3b82f6', '#6366f1', '#8b5cf6', '#ec4899'].map(color => (
                                            <button
                                                key={color}
                                                onClick={() => onUpdateBlock(selectedBlock.id, { styles: { ...styles, borderColor: color } })}
                                                className={`w-5 h-5 rounded-full border ${styles.borderColor === color ? 'ring-2 ring-offset-1 ring-gray-400' : 'border-gray-200'}`}

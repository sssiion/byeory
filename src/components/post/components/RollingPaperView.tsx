...
const RollingPaperView: React.FC<Props> = ({ cycle, onPassTurn }) => {
  ...
  React.useEffect(() => {
    const loadSharedContent = async () => {
      ...
      if (
        (cycle.status === "IN_PROGRESS" && cycle.isMyTurn) ||
        ...
        try {
          ...
          if (data) {
            ...
            const lockedBlocks =
              data.blocks?.map((b: any) => ({ ...b, locked: true })) || [];
            const lockedStickers =
              data.stickers?.map((s: any) => ({ ...s, locked: true })) || [];
            const lockedTexts =
              data.floatingTexts?.map((t: any) => ({ ...t, locked: true })) ||
              [];
            const lockedImages =
              data.floatingImages?.map((i: any) => ({ ...i, locked: true })) ||
              [];
            ...
            if (lockedBlocks.length === 0) {
              editor.setBlocks([
                { id: `new-${Date.now()}`, type: "paragraph", text: "" },
              ]);
            } else {
              editor.setBlocks([
                ...lockedBlocks,
        ...
        <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-2">
          {(() => {
            ...
            return currentMember
              ? `${currentMember.nickname}님 작성 중입니다...`
              : "누군가님이 작성 중입니다...";
          })()}
        </h2>
            ...
            <div className="p-4">
              <EditorSidebar
                ...
                onAddBlock={() =>
                  editor.setBlocks([
                    ...editor.blocks,
                    { id: `m-${Date.now()}`, type: "paragraph", text: "" },
                  ])
                }
                ...
                setSelectedLayoutId={editor.setSelectedLayoutId}
                tempImages={editor.tempImages}
                setTempImages={editor.setTempImages}
                fileInputRef={
                  editor.fileInputRef as React.RefObject<HTMLInputElement>
                }
                handleImagesUpload={handleImagesUpload}
                onAiGenerate={editor.handleAiGenerate}

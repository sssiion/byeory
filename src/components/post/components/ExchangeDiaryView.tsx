...
const FeelingIcon = ({
  ...
  return (
    <button
      ...
      className={`p-1.5 rounded-full transition-all ${
        selected
          ? "bg-indigo-100 text-indigo-500 ring-2 ring-indigo-200"
          : "text-gray-400 hover:bg-gray-100"
      }`}
    >
      <Icon size={18} />
...
const ExchangeDiaryView: React.FC<Props> = ({ cycle, onPassTurn }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!confirm("일기를 제출하시겠습니까? 제출 후에는 수정할 수 없습니다."))
      return;
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)] font-serif italic">
            Shared Exchange Diary
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[url('https://www.transparenttextures.com/patterns/notebook.png')] bg-[#fdfbf7] p-8 rounded-xl shadow-lg border border-[#e5e0d8]">
        {cycle.members?.map((member, index) => {
          ...
          const isMeMatchingCurrentTurn =
            isCurrent && myEmail && member.email === myEmail;
          const canIWrite =
            (cycle.isMyTurn &&
              isCurrent &&
              (member.userId === (cycle.myTurnOrder ?? -1) ||
                member.email === myEmail)) ||
            isMeMatchingCurrentTurn;
          ...
          if (
            ...
            (member.isCompleted && sharedContent)
          ) {
            return (
              <div
                key={itemKey}
                className="aspect-[4/3] bg-white p-5 border-b-2 border-r-2 border-gray-100 shadow-sm flex flex-col rotate-1 hover:rotate-0 transition-transform cursor-pointer"
              >
          ...
          if (cycle.status === "IN_PROGRESS" && canIWrite) {
            return (
              <div
                key={itemKey}
                className="aspect-[4/3] bg-white shadow-xl ring-4 ring-indigo-100 rounded-lg p-5 flex flex-col relative z-10 scale-105 transition-transform"
              >
                <div className="flex flex-col border-b pb-3 mb-4 gap-2">
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-sm text-indigo-900 shrink-0">
                      {member.nickname} 님
                    </span>
          ...
          return (
            <div
              key={itemKey}
              className="aspect-[4/3] border-2 border-dashed border-gray-200 rounded-lg flex flex-col items-center justify-center bg-gray-50/50 opacity-70"
            >
              {isCurrent ? (
                <div className="text-center animate-pulse">
                  <span className="text-2xl mb-2 block">✍️</span>
                  <p className="text-xs font-bold text-gray-400">
                    {member.nickname}님이 작성 중...
                  </p>
                </div>
              ) : member.isCompleted ? (
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-300">
                    {member.order ?? member.turnOrder}번째 순서
                  </p>
                  <p className="text-xs font-bold text-gray-400">
                    {member.nickname}
                  </p>
